...
// 		print_snapshot_result("First Function") // print the profiling
// result for first_func() 		second_func(...);
//		create_snapshot();
// 		print_snapshot_result("First Function") // print the profiling
// result for second_func()
class Profiler {
public:
...
private:
  ...
  static void print_memory(const char *title, const char *verb,
                           int memory_value);
...
private:
  ...
  static int pick_info(const char *info_name);
...
public:
  ...
  // print the snapshot value on the screen with the "title" text to distinguish
  // the data
  void print_snapshot_result(const char *title);

  // print the peak memory during the program execution
  static void print_peak_memory();
};
