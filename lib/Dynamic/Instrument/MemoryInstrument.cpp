...
void Instrumenter::instrumentMain(Function& mainFunc)
{
	...
	// Create calls with empty argument lists to match the void(void) signature
	CallInst::Create(hooks.getInitHook(), ArrayRef<Value*>(), "", pos);
	CallInst::Create(hooks.getGlobalHook(), ArrayRef<Value*>(), "", pos);
	...
	if (mainFunc.arg_size() > 0)
	{
		...
		// Ensure argv is of the right type (char**)
		Type* charPtrPtrType = PointerType::getUnqual(PointerType::getUnqual(Type::getInt8Ty(module.getContext())));
		if (argv->getType() != charPtrPtrType) {
			argv = new BitCastInst(argv, charPtrPtrType, "argv.cast", pos);
		}
			
		auto argvId = getID(argv);
		auto idArg = ConstantInt::get(getIntType(), argvId);

		// Default envp to null if not provided
		Value* envp = ConstantPointerNull::get(cast<PointerType>(charPtrPtrType));
		Constant* envpArg = ConstantInt::get(getIntType(), 0);
		
		// Check if there's an envp argument
		++argItr;
		if (argItr != mainFunc.arg_end())
		{
			envp = argItr;
			// Ensure envp is of the right type (char**)
			if (envp->getType() != charPtrPtrType) {
				envp = new BitCastInst(envp, charPtrPtrType, "envp.cast", pos);
			}
		...
		// Create the call with all arguments properly typed
