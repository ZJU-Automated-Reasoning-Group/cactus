...
static util::Optional<LogRecord> readRecord(std::istream& is)
{
	LogRecord rec;
	...
	if (!succ)
		return util::Optional<LogRecord>();
	else
		return util::make_optional(std::move(rec));
}
...
util::Optional<LogRecord> LazyLogReader::readLogRecord()
{
