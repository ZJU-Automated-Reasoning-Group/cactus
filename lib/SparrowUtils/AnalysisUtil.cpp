...
std::string analysisUtil::getSourceLocOfFunction(const llvm::Function *F)
{
    ...
    if(CU_Nodes) {
        for (unsigned i = 0, e = CU_Nodes->getNumOperands(); i != e; ++i) {
            ...
            for (unsigned j = 0, je = SPs.getNumElements(); j != je; ++j) {
                DISubprogram SP(SPs.getElement(j));
                if (SP.describes(F))
...
std::string analysisUtil::getFileNameOfFunction(const llvm::Function *F) {
    ...
    if (const Instruction *inst = dyn_cast<Instruction>(val)) {
        ...
        if(CU_Nodes) {
            for (unsigned i = 0, e = CU_Nodes->getNumOperands(); i != e; ++i) {
                ...
                for (unsigned j = 0, je = GVs.getNumElements(); j != je; ++j) {
                    DIGlobalVariable GV(GVs.getElement(j));
...
std::string analysisUtil::getFileName(const llvm::Value *val) {
    ...
    if (const Instruction *inst = dyn_cast<Instruction>(val)) {
        if (isa<AllocaInst>(inst)) {
            ...
            if (DDI) {
                DIVariable DIVar(DDI->getVariable());
                rawstr<<DIVar.getFile().getFilename();
            }
        }
        else if (MDNode *N = inst->getMetadata("dbg")) { // Here I is an LLVM instruction
            DILocation Loc(N);
        ...
        rawstr << getFileNameOfFunction(argument->getParent());
    }
    else if (const GlobalVariable* gvar = dyn_cast<GlobalVariable>(val)) {
        NamedMDNode* CU_Nodes = gvar->getParent()->getNamedMetadata("llvm.dbg.cu");
        if(CU_Nodes) {
        ...
        rawstr << getFileNameOfFunction(func);
    }
    else {
        rawstr << "Can only get source location for instruction, argument, global var or function.";
    }
