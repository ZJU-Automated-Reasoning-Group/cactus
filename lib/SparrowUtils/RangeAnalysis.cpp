...
#include "SparrowUtils/RangeAnalysis.h"
...
APInt IntraProceduralRACropDFS::getMin()
{
    return Min;
}

APInt IntraProceduralRACropDFS::getMax()
{
    return Max;
}


Range IntraProceduralRACropDFS::getRange(const Value *v) {
    return CG->getRange(v);
}

bool IntraProceduralRACropDFS::runOnFunction(Function &F) {
    //  if(CG) delete CG;
    CG = new CropDFS();
...
IntraProceduralRACropDFS::~IntraProceduralRACropDFS() {
#ifdef STATS
...
APInt IntraProceduralRACousot::getMin()
{
    return Min;
}

APInt IntraProceduralRACousot::getMax()
{
    return Max;
}


Range IntraProceduralRACousot::getRange(const Value *v) {
    return CG->getRange(v);
}

bool IntraProceduralRACousot::runOnFunction(Function &F) {
    //  if(CG) delete CG;
    CG = new Cousot();
...
IntraProceduralRACousot::~IntraProceduralRACousot() {
#ifdef STATS
    prof.printTime("BuildGraph");
    prof.printTime("Nuutila");
    prof.printTime("SCCs resolution");
...
APInt InterProceduralRACropDFS::getMin()
{
    return Min;
}

APInt InterProceduralRACropDFS::getMax()
{
    return Max;
}


Range InterProceduralRACropDFS::getRange(const Value *v){
    return CG->getRange(v);
}


unsigned InterProceduralRACropDFS::getMaxBitWidth(Module &M) {
    unsigned max = 0;
...
bool InterProceduralRACropDFS::runOnModule(Module &M) {
    // Constraint Graph
    //  if(CG) delete CG;
    CG = new CropDFS();
    ...
    outs() << "Range Finished!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    outs() << "Number of vars:              " << numVars << "\n";
    outs() << "Number of vars [c, +inf]:    " << numCPlusInf << "\n";
    outs() << "Number of vars [c, c]   :    "    << numCC << "\n";
    outs() << "Number of vars [-inf, c]:    " << numMinInfC<< "\n";
    outs() << "Number of vars [-inf, +inf]: " << numMaxRange << "\n";
    outs() << "Number of constants:         " << numConstants << "\n";
    return false;
}


void InterProceduralRACropDFS::MatchParametersAndReturnValues(Function &F,
        ConstraintGraph &G) {
...
InterProceduralRACropDFS::~InterProceduralRACropDFS(){
#ifdef STATS
    prof.printTime("BuildGraph");
    prof.printTime("Nuutila");
    prof.printTime("SCCs resolution");
...
APInt InterProceduralRACousot::getMin()
{
    return Min;
}


APInt InterProceduralRACousot::getMax()
{
    return Max;
}


Range InterProceduralRACousot::getRange(const Value *v){
    return CG->getRange(v);
}


unsigned InterProceduralRACousot::getMaxBitWidth(Module &M) {
    unsigned max = 0;
    // Search through the functions for the max int bitwidth
    for (Module::iterator I = M.begin(), E = M.end(); I != E; ++I) {
        if (!I->isDeclaration()) {
...
bool InterProceduralRACousot::runOnModule(Module &M) {
    // Constraint Graph
    //  if(CG) delete CG;
    CG = new Cousot();
    ...
    outs() << "Number of variables:          " << numVars << "\n";
    outs() << "Number of variables [c, +inf]." << numCPlusInf << "\n";
    outs() << "Number of variables [c, c]."    << numCC << "\n";
    outs() << "Number of variables [-inf, c]: " << numMinInfC<< "\n";
    outs() << "Number of variables [-inf, +inf]. " << numMaxRange << "\n";
    return false;
}


void InterProceduralRACousot::MatchParametersAndReturnValues(Function &F,
        ConstraintGraph &G) {
    // Only do the matching if F has any use
    if (!F.hasNUsesOrMore(1)) {
        return;
...
InterProceduralRACousot::~InterProceduralRACousot(){
#ifdef STATS
    prof.printTime("BuildGraph");
    prof.printTime("Nuutila");
    prof.printTime("SCCs resolution");
 ...
 */
void ConstraintGraph::buildConstantVector(const SmallPtrSet<VarNode*, 32> &component, const UseMap &compusemap)
{
    // Remove all elements from the vector
    constantvector.clear();

    // Get constants inside component (TODO: may not be necessary, since components with more than 1 node may
    ...
    for (UseMap::const_iterator umit = compusemap.begin(), umend = compusemap.end(); umit != umend; ++umit) {
        for (SmallPtrSetIterator<BasicOp*> sit = umit->second.begin(), send = umit->second.end(); sit != send; ++sit) {
