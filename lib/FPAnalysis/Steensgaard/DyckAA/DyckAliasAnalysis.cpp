...
#include "FPAnalysis/Steensgaard/DyckCG/DyckCallGraph.h"
...
static RegisterPass<Steensgaard::DyckAliasAnalysis> X("dyckaa", "Dyck-CFL-Reachability Alias Analysis",true,false);
...
void DyckAliasAnalysis::performDyckAliasAnalysis(Module & M) {
    ...
    clock_t start1, end1;
    start1=clock();
    //outs() << "[Steensgaard] Start intra-procedure analysis...\n";
    ...
    //cout<<"[Steensgaard] Intra-procedure analysis is finished in "<<fixed<<setprecision(2)<<(float)total_time1<<" s"<<endl;
    ...
    clock_t start2, end2;
    start2=clock();
    //outs() << "[Steensgaard] Start inter-procedure analysis...\n";
    ...
    //cout<<"[Steensgaard] Inter-procedure analysis is finished in "<<fixed<<setprecision(2)<<(float)total_time2<<" s"<<endl;
    ...
    //TimeMemProfiler.print_snapshot_result(outs(), "[Steensgaard]");
...
std::set<Value*> DyckAliasAnalysis::collectSteensgaardCluster(Value *calledValue) {
