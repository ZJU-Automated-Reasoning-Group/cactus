...
#include <signal.h>
#include <iostream>
#include "FPAnalysis/Steensgaard/DyckAA/AAAnalyzer.h"


using namespace Steensgaard;

static cl::opt<bool> NoFunctionTypeCheck("sparrow-no-function-type-check", cl::init(false), cl::Hidden,
                                         cl::desc("Do not check function type when resolving pointer calls."));

static cl::opt<bool> WithFunctionCastComb("sparrow-with-function-cast-comb", cl::init(false), cl::Hidden,
                                          cl::desc("Combine compatible functions if there is a cast between the two types."));

static cl::opt<unsigned> NumInterIteration("sparrow-dyckaa-inter-iteration", cl::init(UINT_MAX), cl::Hidden,
                                           cl::desc("The max number of iterators for fix-pointer computation during interprocedure analysis."));

static cl::opt<string> FieldSensitivity("sparrow-canary-field",  cl::init("all"), cl::NotHidden,
                               cl::desc("Please specify the handling of struct fields: no(field-insensitive), all(field-sensitive), selective (default)"));

static cl::opt<string> FuncTypeCheck("sparrow-canary-func-type-check",  cl::init("strong"), cl::NotHidden,
                                        cl::desc("Please specify the handling of checking function types: no, weak, strong (default)"));
...
AAAnalyzer::AAAnalyzer(Module* m, DyckAliasAnalysis* a, DyckGraph* d, DyckCallGraph* cg):progress_bar("[Canary]", ProgressBar::PBS_CharacterStyle){
    module = m;
    aa = a;
    dgraph = d;
    callgraph = cg;
    addrAA=TypeAnalysis::getTypeAnalysis(*m);
    collectFPStructs(*m);
...
void AAAnalyzer::handle_global_variables() {

    for(auto& globalVal: module->globals()){
        if(globalVal.hasDefinitiveInitializer()){
            Constant* c=globalVal.getInitializer();
            this->addPtrTo(wrapValue(&globalVal), wrapValue(c));
        }
    }
}
...
void AAAnalyzer::start_intra_procedure_analysis() {
    ...
    //printf("[Canary] Intra-procedural analysis...");
}
...
void AAAnalyzer::start_inter_procedure_analysis() {
    //printf("[Canary] Inter-procedural analysis...");
}
...
//                progress_bar.showProgress((NumProcessedFunctions + callgraph->size() * (IterationCounter - 1))
...
FieldSBranch:
    ...
    }else if(FieldSensitivity=="no"){

FieldISBranch:

        return current;
    }else{
        Type* gep_ele_type=gep->getPointerOperandType()->getPointerElementType();

        //gep_ele_type->dump();

        // TODO: handing {}* cases
        if(gep_ele_type->isStructTy()){
            if(struct_contained_fp_types_cache.count((StructType*)gep_ele_type)){
                goto FieldSBranch;
            }else{
                goto FieldISBranch;
            }
...
void AAAnalyzer::getCompatibleFunctions(CallInst * icall, set<Function*>& compatibleFuncs) {

    std::set<Function*> callees=addrAA->getMLTAResult(icall);

    for(auto callee:callees){
        compatibleFuncs.insert(callee);
    }
...
FieldISBranch:
        this->makeAlias(toInOrExVal, currentStruct);
    }else{
        if(aggTy->isStructTy()){
            if(struct_contained_fp_types_cache.count((StructType*)aggTy)){
                goto FieldSBranch;
            }else{
                goto FieldISBranch;
            }
...
//    CallInst* icall=(CallInst*)ret;
//    if(addrAA->isIndirectCall(icall)){
//            PointerCall * pcall = new PointerCall(ret, cv, args);
//            parent->addPointerCall(pcall);
//    }
    ...
    if (c->instruction) {
        ...
        if (!((FunctionType*) calledFuncTy)->getReturnType()->isVoidTy()) {
            ...
            while (retIt != rets.end()) {
                ...
                if(val->getType()->isSized()&&retTy->isSized()){
...
//		outs() << "Handling indirect calls in Function #" << FUNCTION_COUNT << "... " << percentage << "%, \r";
        ...
        set<Value*> unhandled_function; // need to be handled

        const set<Value*>* equivSet = aa->getAliasSet(pcall->calledValue);

        // FIXME： checking why the below is significantly slower
//        for(auto itCallee=equivSet->begin();itCallee!=equivSet->end();itCallee++){
//            Value* val=*itCallee;
//            dgraph->updateFPResultValueSet(val);
//        }

        set<Function*>* maycallfuncs = &(pcall->mayAliasedCallees);

        if(FuncTypeCheck=="no"){
            // do nothing for checking types
            for(auto itCallee=equivSet->begin();itCallee!=equivSet->end();itCallee++){
                Value* val=*itCallee;
                if(Function* callee= dyn_cast<Function>(val)){
                    if(!maycallfuncs->count(callee)){
                        unhandled_function.insert(callee);
                    }
                }
            }
            ...
            set<Function*> cands; // functions with compatible types
            set<Value*> equivAndTypeCompSet;  // handle each unhandled, possible function

            if(FuncTypeCheck=="weak"){
                ...
                set<Function*>* cands2 = this->getCompatibleFunctions((FunctionType*) fty);
                cands=*cands2;
            }else{ //FuncTypeCheck=="strong"
                set<Function*> cands2;
                this->getCompatibleFunctions((CallInst*)pcall->instruction,cands2);
                cands=cands2;
            }

            for(auto itCallee=cands.begin();itCallee!=cands.end();itCallee++){
                Function* callee=*itCallee;
                if(equivSet->count(callee)){
                    equivAndTypeCompSet.insert(callee);
                }
            }

            if(maycallfuncs->empty()&&!equivAndTypeCompSet.empty()){
                // for soundness
                for(auto itCallee=equivAndTypeCompSet.begin();itCallee!=equivAndTypeCompSet.end();itCallee++){
                    Value* val=*itCallee;
                    if(Function* callee= dyn_cast<Function>(val)){
                        unhandled_function.insert(callee);
                    }
...
void AAAnalyzer::collectFPStructs(Module &M) {
    ...
    for (StructType *st : M.getIdentifiedStructTypes()) {
        ...
        if (st->isOpaque()) {
            ...
            for (unsigned i = 0; i < n_fields; i++) {
                ...
                unsigned field_bitoffset = st_layout->getElementOffsetInBits(i);
                if (field_type->isPointerTy()){
                    ...
                    }else if (field_type->getPointerElementType()->isFunctionTy()) {
                        add_to_cache = true;
                        break;
                    }
                }
            ...
            if (add_to_cache) {
                struct_contained_fp_types_cache.insert(st);
            }
        }
    }
}
