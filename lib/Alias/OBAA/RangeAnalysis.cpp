...
#include "Alias/OBAA/RangeAnalysis.h"
...
void ConstraintGraph::buildValueBranchMap(const BranchInst *br) {
  ...
  if (constant) {
    ...
    ConstantRange tmpT = CR;
    // In LLVM 3.6.2, there is no built-in method for makeSatisfyingICmpRegion
    // We'll use a simpler approximation based on available methods
    if (pred == CmpInst::ICMP_EQ) {
        tmpT = CR;
    } else if (pred == CmpInst::ICMP_NE) {
        tmpT = CR.inverse();
    } else {
        // For other predicates, use a simplified approach
        tmpT = CR;
    }
